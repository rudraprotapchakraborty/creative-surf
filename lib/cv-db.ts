...
/**
 * Counts CVs generated by this user since `since` — used to enforce the
 * free-tier monthly generation quota. Every generation inserts a new
 * document (there is no in-place "update" path), so this doubles as a
 * generation counter without a separate usage table.
 */
export async function countCvsSince(userId: string, since: Date): Promise<number> {
  const db = await getDb();
  const collection = db.collection(COLLECTION_NAME);
